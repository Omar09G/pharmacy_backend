...
use crate::{
    api_module::vw_sales_with_payments::vw_sales_with_payments_dto::VwSalesWithPaymentsResponse,
    api_utils::{
        api_error::ApiError,
...
pub async fn get_vw_sales_with_payments(
    ...
    let select = schemas::vw_t_sales_with_payments::Entity::find();

    let paginator = select
        .order_by_desc(schemas::vw_t_sales_with_payments::Column::Date)
    ...
    Ok(Json(ApiResponse::success(
        items
            .into_iter()
            .map(VwSalesWithPaymentsResponse::from)
            .collect(),
        "Sales with payments retrieved successfully".to_string(),
        total_items as i32,
    )))
}
