...
use crate::{
    api_module::vw_daily_cash_cut::vw_daily_cash_cut_dto::VwDailyCashCutResponse,
    api_utils::{
        api_error::ApiError,
...
pub async fn get_vw_daily_cash_cut(
    ...
    let select = schemas::vw_t_daily_cash_cut::Entity::find();

    let paginator = select
        .order_by_desc(schemas::vw_t_daily_cash_cut::Column::Day)
    ...
    Ok(Json(ApiResponse::success(
        items
            .into_iter()
            .map(VwDailyCashCutResponse::from)
            .collect(),
        "Daily cash cut retrieved successfully".to_string(),
        total_items as i32,
    )))
}
