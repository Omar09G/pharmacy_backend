...
use crate::{
    api_module::vw_sale_items_detail::vw_sale_items_detail_dto::VwSaleItemsDetailResponse,
    api_utils::{
        api_error::ApiError,
...
pub async fn get_vw_sale_items_detail(
    ...
    // Call fn_t_sale_items_detail(p_sale_id, p_product_id, p_start, p_end)
    let stmt = Statement::from_sql_and_values(
        DatabaseBackend::Postgres,
        "SELECT pharmacy.fn_t_sale_items_detail(0, 0, NULL, NULL)",
        [
    ...
    let select = schemas::vw_t_sale_items_detail::Entity::find();

    let paginator = select
        .order_by_asc(schemas::vw_t_sale_items_detail::Column::SaleItemId)
    ...
    Ok(Json(ApiResponse::success(
        items
            .into_iter()
            .map(VwSaleItemsDetailResponse::from)
            .collect(),
        "Sale items detail retrieved successfully".to_string(),
        total_items as i32,
    )))
}
