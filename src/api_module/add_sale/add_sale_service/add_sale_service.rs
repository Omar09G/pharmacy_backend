...
use crate::{
    ...
    api_utils::{api_response::PaginationParams, api_utils_fun::valite_date_time_range_date},
};
...
pub async fn get_add_sales_with_details(
    ...
    let select = if let (Some(fecha_init), Some(fecha_end)) = (fecha_init, fecha_end) {
        let (fecha_init_dt, fecha_end_dt) = valite_date_time_range_date(fecha_init, fecha_end)?;
    ...
    // Para cada venta, obtener detalles relacionados y mapear a SaleAddDetailResponse
...
//Para cancelar una venta el estatus debe de ser CANCEL y se deben eliminar los items, payment y payment allocation relacionados (o marcar como cancelados si se prefiere mantener el registro)
